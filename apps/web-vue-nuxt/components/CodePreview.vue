...
<template>
  <Teleport to="body">
    <div v-if="visible" class="fixed inset-0 z-50 flex flex-col bg-white animate-slide-up">
      <header class="flex items-center justify-between px-4 h-12 safe-top bg-white hairline">
        ...
        <span class="text-sm font-medium text-gray-800 truncate px-2">{{
      ...
      <div class="flex border-b border-gray-100">
        <button
          ...
          :class="
            activeTab === 'code' ? 'text-brand-600 border-b-2 border-brand-500' : 'text-gray-500'
          "
        ...
        <button
          class="flex-1 py-2 text-sm font-medium transition-colors"
          :class="
            activeTab === 'preview' ? 'text-brand-600 border-b-2 border-brand-500' : 'text-gray-500'
          "
          ...
          <div class="border border-gray-200 rounded-xl overflow-hidden min-h-[300px] bg-gray-50">
            <div class="bg-white p-8 flex items-center justify-center min-h-[280px]">
              ...
              <p v-if="!code" class="text-center text-gray-400">
                <Palette class="w-10 h-10 mx-auto mb-2 text-gray-300" />
                预览功能开发中...
              </p>
            </div>
          </div>
