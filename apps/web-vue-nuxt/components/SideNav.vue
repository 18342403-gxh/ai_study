...
import { Sparkles, BookOpen, Bot, History, Palette, Settings } from 'lucide-vue-next'

const navItems = [
  { key: 'generate', label: '组件生成', icon: Sparkles, to: '/', desc: 'AI 对话生成组件代码' },
  { key: 'rag', label: '知识库', icon: BookOpen, to: '/rag', desc: 'RAG 文档管理与查询' },
  { key: 'agent', label: 'Agent 调试', icon: Bot, to: '/agent', desc: '多步推理与工具调用' },
  { key: 'history', label: '历史会话', icon: History, to: '/history', desc: '多轮对话历史管理' },
  { key: 'library', label: '模板库', icon: Palette, to: '/library', desc: '预设组件模板' },
  { key: 'settings', label: '设置', icon: Settings, to: '/settings', desc: 'API 配置' },
]
  ...
  <aside class="w-60 h-full bg-white border-r border-slate-200 flex flex-col shrink-0">
    ...
    <nav class="flex-1 px-3 space-y-1">
      <NuxtLink
        ...
        :class="
          activeKey === item.key ? 'bg-brand-50 text-brand-700' : 'text-slate-600 hover:bg-slate-50'
        "
      >
        <component
          :is="item.icon"
          class="w-5 h-5"
          :class="activeKey === item.key ? '' : 'opacity-60'"
        />
        <div class="flex-1 min-w-0">
          <p class="text-sm font-medium truncate">{{ item.label }}</p>
          <p
            class="text-[11px] truncate"
            :class="activeKey === item.key ? 'text-brand-400' : 'text-slate-400'"
          >
            {{ item.desc }}
          </p>
        </div>
        <span v-if="activeKey === item.key" class="w-1.5 h-1.5 rounded-full bg-brand-500" />
      </NuxtLink>
    </nav>
    ...
    <div class="px-5 py-4 border-t border-slate-100">
      <div class="flex items-center gap-2">
        <div
          class="w-7 h-7 rounded-full bg-slate-100 flex items-center justify-center text-xs text-slate-500"
        >
          AI
        </div>
        <div class="flex flex-col">
